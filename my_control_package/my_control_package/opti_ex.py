...
class TaskNode(Node):
    ...
    def costo_funzionale(self,q_dot, q_dot_des):

        return np.linalg.norm((q_dot-q_dot_des)**2)
    
    def vincolo_limite_massimo(self, q_dot, q_max):
        
        return q_max - q_dot  # Deve essere >= 0
    
    def qdot1_0(self,q_dot,q_value):
        
        return q_dot[5]-q_value
    ...
    def on_timer(self):
        ...
        q_dot_des= np.linalg.pinv(J) @ twist_command
        ...
        if risultato.success:
            ...
            self.get_logger().warn('Ottimizzazione SciPy fallita!')

        twist_command_opt= J @ q_dot_ottimo

        twist_msg.linear.x=float(twist_command_opt[0])
        twist_msg.linear.y=float(twist_command_opt[1])
        twist_msg.linear.z=float(twist_command_opt[2])
        twist_msg.angular.x=float(twist_command_opt[3])
        twist_msg.angular.y=float(twist_command_opt[4])
        twist_msg.angular.z=float(twist_command_opt[5])
    ...
    #prelevo i valori dei giunti attuali (vettore np)
    def listener_callback(self,msg):
        ...
        self.qdot=np.array(qdot_list)




        
    #Crep il modello Pinocchio
