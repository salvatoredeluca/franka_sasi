# Custom ROS 2 Integration for Franka FR3 bimanual teleoperation setup

## About
This repository relies on **franka_ros2** and consists in a customization including a unified setup for simulation and real hardware, a custom impedance controller, and a custom bimanual teleoperation framework.

## Docker Container Installation

1. **Clone the Repositories:**
    ```bash
    git clone https://github.com/romotive-project/franka-stack.git
    cd franka-stack
    ```
...
You can customize the configuration changing the parameters in `franka_bringup/config/franka.config.yaml`. 
### Bring up simulation or real hardware
- If you want to run the code for debbugging, set `use_fake_hardware: "true"`, `gazebo: "false"`, `use_rviz: "true`.
- If you want to run the code in simulation, set `use_fake_hardware: "true"`, `gazebo: "true"`, `use_rviz: "true`.
- If you want to run the code on real hardware, set `use_fake_hardware: "false"` and `use_rviz: "false`
### Controller
You can use the custom impedance controller setting `controller: "custom_controller"`. The controller gains are editable in `franka_bringup/config/franka.config.yaml`
#### Controller topics
When using the custom controller, topics interfaces are available. The desired twist for each robot referred in each robot's base link frame should be published as a `geometry_msgs/msg/Twist` message on the topics `/left/ee_twist_command` and `/right/ee_twist_command`, respectively. In the current version, twist commands are working in simulation and still have to be tested on the real hardware.
#### Controller services
The gripper commands for each robots should be commanded as a `std_msgs/msg/Bool` message on the services `/left/gripper_close_command` and `/right/gripper_close_command` (true closes the gripper, false opens it). In the current version, the gripper is not working in simulation, while it has still to be tested on real hardware.


## Run Haption Virtuose 6D
1. **Check connection with Haptic devices**
    ```bash
    ping 192.168.9.16
    ```
    ```bash
...
2. **Calibrate Haptic devices**
    ```bash
    sudo /ros2_ws/src/6DOFs_haptic_devices/n71/7.2.0000/calibrate_Desktop_6D_n71
    ```
    ```bash
    sudo /ros2_ws/src/6DOFs_haptic_devices/n70/7.2.0000/calibrate_Desktop_6D_n70
    ```

3. **Launch dual haptic**
    ```bash
    ros2 launch haptic_control dual_haptic.launch.py
    ```

4. **Launch teleoperation node**
    ```bash
    ros2 launch haptic_control haptic_teleop.launch.py
    ```


## Record a LeRobot Dataset

The `franka_dataset/record_lerobot_dataset.py` node records a dual Franka FR3 teleoperation session into a [LeRobot](https://github.com/huggingface/lerobot) dataset compatible with openpi (pi0 / pi0-FAST) fine-tuning.

### Prerequisites

Make sure the following topics are being published before starting the recorder:

| Topic | Type | Source |
|---|---|---|
| `/left/ee_pose` | `geometry_msgs/PoseStamped` | franka bringup |
| `/left/ee_twist_command` | `geometry_msgs/TwistStamped` | haptic teleop |
| `/left/gripper_status` | `std_msgs/Bool` | franka bringup |
| `/right/ee_pose` | `geometry_msgs/PoseStamped` | franka bringup |
| `/right/ee_twist_command` | `geometry_msgs/TwistStamped` | haptic teleop |
| `/right/gripper_status` | `std_msgs/Bool` | franka bringup |
| `/camera_left/color/image_raw` | `sensor_msgs/Image` | RealSense / camera driver |
| `/camera_right/color/image_raw` | `sensor_msgs/Image` | RealSense / camera driver |


### Launch the recorder

```bash
python3 /ros2_ws/src/franka_dataset/record_lerobot_dataset.py \
    --repo_id YOUR_HF_USERNAME/your-dataset-name \
...
```
python3 /ros2_ws/src/franka_dataset/record_lerobot_dataset.py \
    --repo_id carmariniello/pickandplace \
    --output_dir ./lerobot_data \
    --fps 30 \
    --image_size 224
```
...
|---|---|---|
| `--repo_id` | `local/test-dataset` | HuggingFace dataset repo ID. Use `local/...` to skip Hub push. |
| `--output_dir` | `./lerobot_data` | Local directory where the dataset is saved. |
| `--fps` | `30` | Recording frequency in Hz. Must match the openpi `TrainConfig`. |
| `--image_size` | `224` | Images are resized to `image_size × image_size` pixels. |

### Keyboard controls

```
Enter        →  start a new episode
Enter        →  stop current episode (prompts for task description)
q + Enter    →  quit and save the dataset
```

### Push to HuggingFace Hub

If `--repo_id` does not start with `local/`, the dataset is automatically pushed to the Hub after quitting. Authenticate first:

```bash
hf auth login
# paste your token from huggingface.co/settings/tokens (write permission required)
```
...
ds = LeRobotDataset("YOUR_HF_USERNAME/your-dataset-name", root="./lerobot_data", local_files_only=True)
ds.push_to_hub()
```


## Development instructions
BEFORE start working, developers should always pull
```bash
git pull
``` 
To update changes in the monorepo, if there are no conflicts, each developer can simply do
```bash
git add --all
...
git push
```
If developers modify the same files at the same time, conflicts should be manually resolved when merging.
